...
const TEST_CASES: TestCase[] = [
  ...
  // === Kategori 3: Konteks Tidak Jelas ===
  ...
  // === Kategori 4: Huruf Alay / Capslock ===
  ...
  // === Kategori 13: Di Luar Knowledge Base ===
...
interface TestResult {
  ...
  behavior: string;
  response: string;
  status: 'LULUS' | 'PERLU_PERBAIKAN' | 'GAGAL' | 'TIMEOUT';
  reason: string;
}
...
async function flushPoll(): Promise<string[]> {
  try {
    const res = await axios.get(POLL_URL);
    const messages = res.data?.data || res.data;
    if (messages && Array.isArray(messages) && messages.length > 0) {
      return messages.map((m: any) => m.text);
    }
  } catch (e) {}
  return [];
}

async function sendAndWait(question: string): Promise<string> {
  await flushPoll(); // flush leftovers
  ...
  // No debounce now (0ms), just wait for AI processing (max 30s)
  const maxWait = 30000;
...
function evaluateResponse(tc: TestCase, response: string): { status: TestResult['status'], reason: string } {
  ...
  // Default: Typo, Singkatan, Alay, Campur Kode, Multi-Intent, Bahasa Daerah, Pesan Panjang
