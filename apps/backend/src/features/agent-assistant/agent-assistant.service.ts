...
export class AgentAssistantService {
  ...
  async chatWithOwnerAssistant(
    ...
    try {
      this.logger.log(
        `[AGENT-ASSISTANT] Asisten Owner mencari context untuk tenantId: ${tenantId}, query: "${text}"`,
      );

      // Gunakan RAG Search untuk mencari konteks yang hanya relevan dengan text (query)
      const relevantContext = await this.ragService.searchRelevantContext(
        text,
        10,
        tenantId,
      );
      context = relevantContext
        ...
        : '';
      this.logger.log(
        `[AGENT-ASSISTANT] RAG Context Ditemukan (Panjang: ${context.length})`,
      );
    } catch (e) {
      this.logger.warn(
        'Gagal mengambil konteks untuk asisten owner: ' + e.message,
      );
    }
    ...
    if (tenantId) {
      try {
        ...
        if (tenant) {
          const kelengkapanToko: string[] = [];
          if (totalProduk === 0)
            kelengkapanToko.push(
              '- Katalog Produk (Kirim foto brosur/excel/tulis manual)',
            );
...
Contoh: "Halo Bos! Selamat datang. Agar bot CS kita bisa jalan, yuk kita lengkapi datanya dulu. Saat ini kita belum punya Katalog Produk nih, Bos bisa kirim fotonya ke sini..."`;
    ...
    const systemPrompt = `Anda adalah Asisten Pribadi cerdas untuk pemilik UMKM.
...
================================`;
    ...
    const tools = [
      {
        ...
        function: {
          name: 'record_sale',
          description:
            'Mencatat penjualan produk, mengurangi stok, dan merekam omset ke sistem. Gunakan ini jika owner melapor ada barang yang laku/terjual.',
          parameters: {
            type: 'object',
            properties: {
              product_name: {
                type: 'string',
                description:
                  'Nama produk yang terjual, misalnya "Sepatu Sneakers Aerostep V1"',
              },
              quantity: {
                type: 'integer',
              ...
              variant: {
                type: 'string',
                description:
                  'Wajib diisi jika produk memiliki ukuran, warna, atau tipe. Jika owner tidak menyebutkan varian, tanyakan kembali.',
              },
              notes: {
                type: 'string',
      ...
      },
      {
        ...
        function: {
          ...
          description:
            'Membuat filter untuk menarik laporan penjualan (omset/qty). Panggil ini untuk menjawab SEMUA pertanyaan seputar laporan, rekap, omset, penjualan, atau histori. Semua parameter BERSIFAT OPSIONAL. Kosongkan parameter jika user bertanya secara umum (misal: "kapan saja barang laku?").',
          parameters: {
            type: 'object',
            properties: {
              ...
              productName: {
                type: 'string',
                description: 'Nama produk jika mencari spesifik',
              },
...

      {
        ...
        function: {
          name: 'delete_product',
          description:
            'Menghapus produk dari database. Gunakan tool ini jika pemilik secara eksplisit meminta untuk menghapus produk tertentu.',
          parameters: {
            type: 'object',
            properties: {
              product_name: {
                type: 'string',
                description:
                  'Nama produk yang ingin dihapus, misalnya "Sepatu Jogging Ultra Pro"',
              },
            },
      ...
      },
      {
        ...
        function: {
          ...
          description:
            'Mengubah harga semua produk secara massal berdasarkan persentase atau nominal tetap. Gunakan jika owner bilang "naikkan semua harga 10%" atau "turunkan semua harga 5000".',
          parameters: {
            type: 'object',
            properties: {
              type: {
                ...
                description:
                  'Tipe perubahan: percentage (persen) atau fixed (nominal)',
              },
              amount: {
                type: 'number',
                description:
                  'Nilai perubahannya (misal 10 untuk 10%, atau 5000 untuk Rp5.000). Gunakan minus jika diskon/turun.',
              },
            },
      ...
      },
      {
        ...
        function: {
          name: 'update_product',
          description:
            'Mengubah atau mengupdate data produk di database (seperti harga, stok, deskripsi, atau kategori). Gunakan tool ini jika pemilik meminta untuk mengubah data.',
          parameters: {
            type: 'object',
            properties: {
              product_name: {
      ...
      },
      {
        ...
        function: {
          ...
          description:
            'Mengubah atau mengisi informasi profil toko (alamat, jam buka, kebijakan retur). Gunakan tool ini jika pemilik memberikan informasi toko tersebut.',
          parameters: {
            type: 'object',
            properties: {
              address: {
      ...
      },
      {
        ...
        function: {
          name: 'update_cs_bot',
          description:
            'Mengatur identitas bot Customer Service (CS) seperti nama, gaya bahasa, dan kontak darurat. Gunakan ini jika pemilik meminta untuk mengatur bot CS.',
          parameters: {
            type: 'object',
            properties: {
              agentName: {
                type: 'string',
                description:
                  'Nama untuk bot CS, misal "Mbak Mila", "Budi", dll',
              },
              agentTone: {
                type: 'string',
                description:
                  'Gaya bahasa bot CS, misal "ramah", "anak jaksel", "formal"',
              },
              fallbackContact: {
                type: 'string',
                description:
                  'Nomor telepon atau kontak darurat jika bot tidak bisa menjawab',
              },
            },
          },
        },
    ...
    try {
      this.logger.log(
        `[AGENT-ASSISTANT] Mengirim request ke OpenAI untuk Owner Assistant...`,
      );
      ...
      if (responseMessage.tool_calls && responseMessage.tool_calls.length > 0) {
        this.logger.log(
          `[AGENT-ASSISTANT] OpenAI memanggil tool(s): ${responseMessage.tool_calls.map((t: any) => t.function?.name).join(', ')}`,
        );
        ...
        for (const toolCall of responseMessage.tool_calls as any[]) {
          ...
          try {
            ...
            this.logger.error(
              `AI Hallucination: Gagal parse JSON argumen tool ${toolCall.function.name}`,
            );
            messages.push({
              ...
              name: toolCall.function.name,
              content:
                'Error: Argumen yang Anda berikan bukan JSON yang valid. Tolong perbaiki format JSON-nya.',
            });
            continue;
          }
          ...
          if (
            ...
            toolCall.function.name === 'record_sale'
          ) {
            const { product_name, quantity, customer_name, notes, variant } =
              args;
            this.logger.log(
              `[AGENT-ASSISTANT] Mengeksekusi record_sale untuk "${product_name}" sebanyak ${quantity} pcs.`,
            );
            ...
            try {
              ...
              if (product) {
                const updatedAttributes = product.attributes as any;
                let variantMessage = '';

                if (
                  variant &&
                  updatedAttributes &&
                  typeof updatedAttributes === 'object' &&
                  updatedAttributes.variants
                ) {
                  if (updatedAttributes.variants[variant] !== undefined) {
                    updatedAttributes.variants[variant] = Math.max(
                      0,
                      updatedAttributes.variants[variant] - quantity,
                    );
                ...
                const finalVariantStock =
                  variant &&
                  updatedAttributes?.variants?.[variant] !== undefined
                    ? updatedAttributes.variants[variant]
                    : null;
                const sisaTeks =
                  finalVariantStock !== null
                    ? `Sisa stok varian ${variant}: ${finalVariantStock}`
                    : `Sisa stok total: ${product.stock - quantity}`;
              ...
              this.logger.error(
                `Database error saat record_sale: ${dbError.message}`,
              );
            ...
            toolCall.type === 'function' &&
            toolCall.function.name === 'build_sales_query'
          ) {
            const { startDate, endDate, productName, startHour, endHour } =
              args;
            this.logger.log(
              `[AGENT-ASSISTANT] Mengeksekusi build_sales_query dengan args: ${JSON.stringify(args)}`,
            );
            ...
            try {
              ...
              const aggResult = await this.prisma.$queryRawUnsafe<any[]>(
                aggQuery,
                ...params,
              );
              ...
              const listQuery = `
                ...
                LIMIT 50
              `;
              const recentRecords = await this.prisma.$queryRawUnsafe<any[]>(
                listQuery,
                ...params,
              );
              ...
              this.logger.error(
                `Database error saat build_sales_query: ${dbError.message}`,
              );
              toolResponseContent = JSON.stringify({
            ...
            toolCall.type === 'function' &&
            toolCall.function.name === 'delete_product'
          ) {
            const { product_name } = args;
            this.logger.log(
              `[AGENT-ASSISTANT] Mengeksekusi delete_product untuk "${product_name}"`,
            );
            ...
            try {
              ...
              if (product) {
                const salesCount = await this.prisma.salesRecord.count({
              ...
              this.logger.error(
                `Database error saat delete_product: ${dbError.message}`,
              );
            ...
            toolCall.type === 'function' &&
            toolCall.function.name === 'update_product'
          ) {
            const {
              product_name,
              new_price,
            ...
            this.logger.log(
              `[AGENT-ASSISTANT] Mengeksekusi update_product untuk "${product_name}"`,
            );
            ...
            try {
              ...
              this.logger.error(
                `Database error saat update_product: ${dbError.message}`,
              );
            ...
            toolCall.type === 'function' &&
            toolCall.function.name === 'bulk_update_price'
          ) {
            const { type, amount } = args;
            this.logger.log(
              `[AGENT-ASSISTANT] Mengeksekusi bulk_update_price tipe ${type} sebesar ${amount}`,
            );
            ...
            try {
              if (type === 'percentage') {
                ...
                await this.prisma.$executeRawUnsafe(
                  `UPDATE products SET price = price * ${multiplier} WHERE tenant_id = $1`,
                  tenantId,
                );
              } else {
                await this.prisma.$executeRawUnsafe(
                  `UPDATE products SET price = price + ${amount} WHERE tenant_id = $1`,
                  tenantId,
                );
              }
              await this.dataAgentService.syncKnowledgeBase(tenantId);
              ...
              this.logger.error(
                `Database error saat bulk_update_price: ${dbError.message}`,
              );
              toolResponseContent = `Gagal mengubah data dari database karena terjadi kesalahan internal.`;
            }
            ...
            toolCall.type === 'function' &&
            toolCall.function.name === 'update_store_info'
          ) {
            ...
            try {
              ...
              this.logger.error(
                `Database error saat update_store_info: ${dbError.message}`,
              );
            ...
            toolCall.type === 'function' &&
            toolCall.function.name === 'update_cs_bot'
          ) {
            ...
            try {
              ...
              this.logger.error(
                `Database error saat update_cs_bot: ${dbError.message}`,
              );
        ...
        this.logger.log(
          `[AGENT-ASSISTANT] Mengirim ulang ke OpenAI dengan hasil eksekusi tool...`,
        );
      ...
      const finalResponse =
        responseMessage?.content || 'Saya siap membantu Anda, Bos.';
