  ...
  async getCatalogContext(tenantId?: string): Promise<string> {
    try {
      const products = await this.prisma.product.findMany({
        ...
        select: { name: true, price: true, category: true, description: true, imageUrl: true, stock: true, attributes: true }
      });
      ...
      products.forEach((p, idx) => {
        let variantStr = `Stok Utama di DB: ${p.stock}`;
        if (p.attributes && typeof p.attributes === 'object' && 'variants' in p.attributes) {
          const variants = (p.attributes as any).variants;
          const varDetails = Object.entries(variants).map(([k, v]) => `${k} sisa ${v}`).join(', ');
          variantStr += ` | Detail Varian: ${varDetails}`;
        }
        
        catalogText += `${idx + 1}. ${p.name} (Kategori: ${p.category}) - Harga: Rp ${p.price} - ${variantStr}\n`;
        if (p.description) catalogText += `  Deskripsi: ${p.description}\n`;
        if (p.imageUrl) catalogText += `  [GAMBAR: ${p.name}] ${p.imageUrl}\n`;
      });
