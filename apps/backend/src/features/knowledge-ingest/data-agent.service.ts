...
export class DataAgentService {
  ...
  async syncKnowledgeBase(
    ...
    this.logger.log(
      `Starting manual sync of master data to vector database${tenantId ? ' for tenant ' + tenantId : ''}...`,
    );
    ...
    try {
      ...
      for (const product of products) {
        try {
          ...
          `,
            ...
            content,
            vectorString,
            product.tenantId || null,
          );
          inserted++;
        } catch (err) {
          this.logger.error(
            `Failed to sync product ${product.id}: ${err.message}`,
          );
          errors++;
        }
      }
      ...
      for (const kb of kbs) {
        try {
          ...
          await this.prisma.$executeRawUnsafe(
            ...
            VALUES (gen_random_uuid(), $1, $2, $3::vector, $4, CURRENT_TIMESTAMP)
          `,
            `KnowledgeBase: ${(kb.metadata as any)?.title || kb.id}`,
            kb.content,
            vectorString,
            kb.tenantId || null,
          );
      ...
      if (tenantId) {
        ...
        if (tenant) {
          ...
          const tEmbedding =
            await this.ragService.generateEmbedding(tenantInfo);
      ...
      this.logger.log(
        `Sync completed. Inserted: ${inserted}, Errors: ${errors}`,
      );
      return { inserted, errors };
    } catch (error) {
      this.logger.error(
        `Critical error during knowledge base sync: ${error.message}`,
      );
