...
var DesignFlowService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.DesignFlowService = void 0;
const common_1 = require("@nestjs/common");
const design_ai_service_1 = require("./design-ai.service");
const design_image_service_1 = require("./design-image.service");
const design_session_service_1 = require("./design-session.service");
...
let DesignFlowService = DesignFlowService_1 = class DesignFlowService {
    aiService;
    imageService;
    sessionService;
    logger = new common_1.Logger(DesignFlowService_1.name);
    constructor(aiService, imageService, sessionService) {
    ...
    async generateAndSendImage(bot, userId, session) {
        ...
        const referensiImages = data.referensi_images || [];
        const instruksiUser = data.summary_id || data.summary || '';
        if (base64Aset && referensiImages.length > 0) {
            await bot.sendMessage(userId, '⏳ *Sedang menganalisis gambar aset & referensi dengan GPT-4o, lalu membuat poster...*\n\nProses ini memakan waktu 60–120 detik. Mohon tunggu ya! 🙏', { parse_mode: 'Markdown' });
            const base64Referensi = referensiImages[0];
            const imageSize = data.image_size || '1024x1536';
            const result = await this.imageService.generateWithReference(base64Aset, base64Referensi, instruksiUser, imageSize);
            if (result.success) {
                const imageBuffer = this.imageService.base64ToBuffer(result.imageBase64);
                const publicUrl = await this.sessionService.saveAssetToSupabase(userId, result.imageBase64, { prompt: result.generatedPrompt });
                await bot.sendPhoto(userId, imageBuffer, {
                    caption: `✅ *Poster berhasil dibuat!*\n\n` +
                ...
                await this.sessionService.saveGeneration(userId, result.generatedPrompt ?? null, publicUrl, 'with_reference');
                ...
                await this.sessionService.saveGeneration(userId, instruksiUser, null, 'with_reference', 'failed');
            }
            return;
        }
        await bot.sendMessage(userId, '⏳ *Sedang membuat desainmu...*\n\nMohon tunggu 30–60 detik ya!', { parse_mode: 'Markdown' });
        ...
        if (result.success) {
            const imageBuffer = this.imageService.base64ToBuffer(result.imageBase64);
            const publicUrl = await this.sessionService.saveAssetToSupabase(userId, result.imageBase64, { prompt: finalPrompt });
            await bot.sendPhoto(userId, imageBuffer, {
                caption: `✅ *Desainmu sudah jadi!*\n\n` +
                    `Mau mulai desain baru? Ketik /baru\nIngin perbaiki desain ini? Ketik *revisi*`,
                parse_mode: 'Markdown',
            });
            ...
            await this.sessionService.saveGeneration(userId, finalPrompt, null, 'freeform', 'failed');
        }
    }
    async handle(bot, msg) {
        ...
        if (text === '/start' || text === '/baru') {
            ...
            await bot.sendMessage(userId, '🎨 *Halo! Selamat datang di Design Bot!*\n\nSilakan berikan deskripsi atau prompt bebas tentang desain poster yang ingin kamu buat.', { parse_mode: 'Markdown' });
            return;
        }
        let session = await this.sessionService.getSession(userId);
        if (!session || this.sessionService.isExpired(session)) {
            await this.sessionService.createSession(userId);
            await bot.sendMessage(userId, '🎨 *Halo!* Sesi kamu telah berakhir. Silakan berikan deskripsi desain yang ingin kamu buat.', { parse_mode: 'Markdown' });
            return;
        }
        ...
        if (currentStep === 'prompting') {
            ...
            await bot.sendMessage(userId, buildConfirmationMenu(summaryResult.summary_id, summaryResult.english_prompt), { parse_mode: 'Markdown' });
            return;
        }
        if (currentStep === 'konfirmasi') {
            ...
            if (lowerText === 'revisi') {
                ...
                await bot.sendMessage(userId, '✏️ *Silakan masukkan teks revisi kamu.*\n\nContoh: "Tambahkan harga 500 juta, warna lebih cerah"\n\nSetelah kamu ketik, kesimpulan akan diperbarui otomatis.', { parse_mode: 'Markdown' });
                return;
            }
            if (['aset', 'foto', 'foto rumah', 'foto properti', 'punya foto', 'ada foto'].includes(lowerText)) {
                ...
                await bot.sendMessage(userId, '🏠 *Kirimkan foto rumah/properti* yang akan menjadi hero poster.\n\n' +
                    'Boleh tambahkan catatan teks (harga, keterangan, dll.).\n' +
                    'Kalau sudah, ketik *sudah* untuk kembali ke kesimpulan.', { parse_mode: 'Markdown' });
                return;
            }
            if (['referensi', 'punya referensi', 'ada referensi'].includes(lowerText)) {
                ...
                await bot.sendMessage(userId, '🖼️ *Kirimkan gambar referensi desain poster* yang ingin ditiru layoutnya.\n\n' +
                    'Boleh kirim lebih dari satu gambar dan tambahkan catatan teks.\n' +
                    'Kalau sudah, ketik *sudah* untuk kembali ke kesimpulan.', { parse_mode: 'Markdown' });
                return;
            }
            await bot.sendMessage(userId, '❓ Pilihan tidak dikenali. Silakan balas dengan salah satu:\n\n' +
                '✅ *sudah* → generate gambar sekarang\n' +
                '✏️ *revisi* → ubah deskripsi\n' +
                '🏠 *aset* → kirim foto properti\n' +
                '🖼️ *referensi* → kirim gambar referensi desain', { parse_mode: 'Markdown' });
            return;
        }
        if (currentStep === 'menunggu_revisi') {
            ...
            await bot.sendMessage(userId, buildConfirmationMenu(newSummaryResult.summary_id, newSummaryResult.english_prompt, 'Kesimpulan desainmu yang direvisi:'), { parse_mode: 'Markdown' });
            return;
        }
        if (currentStep === 'menunggu_aset') {
            const lowerText = text ? text.toLowerCase().trim() : '';
            let asetImage = sessionData.aset_image || null;
            const asetNotes = sessionData.aset_notes || [];
            if (lowerText === 'sudah' || lowerText === 'done' || lowerText === 'selesai') {
                if (!asetImage) {
                    await bot.sendMessage(userId, '❌ Kamu belum mengirimkan foto properti. Silakan kirim dulu, atau ketik /baru untuk batal.');
                    return;
                }
                ...
                const newSummaryResult = await this.aiService.summarizePrompt(`Foto properti/aset telah diterima. ${catatanAset ? 'Catatan user: ' + catatanAset : 'Gunakan foto properti sebagai hero image poster.'}`, konteks);
                const updatedData = { ...sessionData, aset_image: asetImage, aset_notes: asetNotes };
                if (newSummaryResult) {
                ...
                await bot.sendMessage(userId, buildConfirmationMenu(summaryToShow, promptToShow, 'Kesimpulan diperbarui dengan foto aset:'), { parse_mode: 'Markdown' });
                return;
            }
            ...
            if (photo && photo.length > 0) {
                ...
                try {
                    ...
                    if (!asetImage) {
                        ...
                        await bot.sendMessage(userId, '✅ *Foto properti diterima!*\n\n' +
                            '📝 Boleh tambahkan catatan teks (harga, keterangan khusus, dll.) — opsional\n\n' +
                            'Kalau sudah, ketik *sudah* untuk kembali ke kesimpulan.', { parse_mode: 'Markdown' });
                    }
                    else {
                        await bot.sendMessage(userId, '⚠️ Foto aset sudah ada. Jika kamu ingin menambahkan *gambar referensi desain*, ' +
                            'silakan kembali ke menu konfirmasi dengan ketik *sudah*, lalu pilih *referensi*.', { parse_mode: 'Markdown' });
                    }
                    return;
                }
                catch (e) {
            ...
            await bot.sendMessage(userId, asetImage
                ? '✅ _Catatan dicatat!_ Ketik *sudah* kalau sudah selesai, atau tambahkan catatan lagi.'
                : '📸 _Catatan dicatat!_ Silakan kirim foto properti/rumah kamu.', { parse_mode: 'Markdown' });
            return;
        }
        if (currentStep === 'menunggu_referensi') {
            const lowerText = text ? text.toLowerCase().trim() : '';
            const referensiNotes = sessionData.referensi_notes || [];
            const referensiImages = sessionData.referensi_images || [];
            if (lowerText === 'sudah' || lowerText === 'done' || lowerText === 'selesai') {
                if (referensiNotes.length === 0 && referensiImages.length === 0) {
                    await bot.sendMessage(userId, '❌ Kamu belum memberikan gambar atau catatan apapun. Silakan kirim referensi desainnya, atau ketik /baru untuk batal.');
                    return;
                }
                ...
                const prevContext = sessionData.summary_id || sessionData.summary;
                const newSummaryResult = await this.aiService.analyzeMultipleImagesAndSummarize(prevContext, referensiImages, combinedNotes);
                if (!newSummaryResult) {
                ...
                await bot.sendMessage(userId, buildConfirmationMenu(newSummaryResult.summary_id, newSummaryResult.english_prompt, 'Kesimpulan diperbarui dengan referensi visual:'), { parse_mode: 'Markdown' });
                return;
            }
            if (text)
                referensiNotes.push(text);
            if (photo && photo.length > 0) {
                const fileId = photo[photo.length - 1].file_id;
                try {
                    const fileLink = await bot.getFileLink(fileId);
            ...
            await bot.sendMessage(userId, `✅ _${referensiImages.length > 0 ? `${referensiImages.length} gambar referensi` : 'Catatan'} diterima!_ ` +
                `Kirim lagi atau ketik *sudah* kalau sudah selesai.`, { parse_mode: 'Markdown' });
            return;
        }
        if (currentStep === 'selesai') {
            if (text && text.toLowerCase().trim() === 'revisi') {
                ...
                await bot.sendMessage(userId, '✏️ *Silakan masukkan teks revisi kamu.*\n\nContoh: "Ganti warna latar menjadi biru, tambah harga 800 juta"\n\nKesimpulan akan diperbarui otomatis.', { parse_mode: 'Markdown' });
                return;
            }
            await bot.sendMessage(userId, '✅ Desain sebelumnya sudah selesai.\n\n' +
                'Ketik /baru untuk membuat desain baru.\n' +
                'Atau ketik *revisi* jika ingin memperbaiki desain terakhir.', { parse_mode: 'Markdown' });
        }
    }
};
exports.DesignFlowService = DesignFlowService;
exports.DesignFlowService = DesignFlowService = DesignFlowService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [design_ai_service_1.DesignAiService,
        design_image_service_1.DesignImageService,
        design_session_service_1.DesignSessionService])
], DesignFlowService);
//# sourceMappingURL=design-flow.service.js.map